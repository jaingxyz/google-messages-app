...
const dumpButtons = async () =>
  page.evaluate(() => {
    const inOverlay = [...document.querySelectorAll(".cdk-overlay-container button, mat-dialog-container button, [role='dialog'] button, [role='menuitem']")];
    return inOverlay.map((el) => ({
...
for (let i = 0; i < count; i++) {
  const nm = (await items.nth(i).locator("[data-e2e-conversation-name], h2.name, .name").first().textContent().catch(() => ""))?.trim();
  if ((nm || "").toLowerCase() === NAME.toLowerCase()) { target = items.nth(i); break; }
}
if (!target) { console.log(JSON.stringify({ error: `no exact match for "${NAME}"` })); await ctx.close(); process.exit(0); }
...
await page.locator("[role='menuitem']", { hasText: /move to trash/i }).first().click();
...
let confirmed = null;
const confirmBtn = page.locator("mat-dialog-container button, [role='dialog'] button", { hasText: /trash|delete|confirm|ok|yes/i }).first();
...
for (let i = 0; i < c2; i++) {
  const nm = (await items.nth(i).locator("[data-e2e-conversation-name], h2.name, .name").first().textContent().catch(() => ""))?.trim();
  if ((nm || "").toLowerCase() === NAME.toLowerCase()) { stillThere = true; break; }
}

console.log(JSON.stringify({ name: NAME, menu, afterTrashClick, confirmedButton: confirmed, stillThere }, null, 2));
await ctx.close();
